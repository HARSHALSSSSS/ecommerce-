...
class DatabaseWrapper {
  ...
  // Execute query and return result with lastID and changes
  ...
  private convertPlaceholders(sql: string): string {
    ...
    // Also convert SQLite-specific syntax
    result = this.convertSQLiteToPostgres(result);
    return result;
  }
  ...
  private convertSQLiteToPostgres(sql: string): string {
    ...
    // Convert datetime('now', '+X minutes') to NOW() + INTERVAL 'X minutes'
    result = result.replace(/datetime\s*\(\s*'now'\s*,\s*'\+(\d+)\s+(minutes?|hours?|days?)'\s*\)/gi, 
      (match, num, unit) => `NOW() + INTERVAL '${num} ${unit}'`);
    
    // Convert datetime('now', '-X days') etc
    result = result.replace(/datetime\s*\(\s*'now'\s*,\s*'-(\d+)\s+(minutes?|hours?|days?)'\s*\)/gi,
      (match, num, unit) => `NOW() - INTERVAL '${num} ${unit}'`);
    
    // Convert datetime(CURRENT_TIMESTAMP, '+X hours')
    result = result.replace(/datetime\s*\(\s*CURRENT_TIMESTAMP\s*,\s*'\+(\d+)\s+(minutes?|hours?|days?)'\s*\)/gi,
      (match, num, unit) => `CURRENT_TIMESTAMP + INTERVAL '${num} ${unit}'`);
    
    // Convert datetime('now', '-' || ? || ' hours') to NOW() - INTERVAL (? || ' hours')
    ...
    // Convert TEXT to TEXT (no change)
    ...
    if (sql.match(/INSERT\s+OR\s+IGNORE/i) && !result.includes('ON CONFLICT')) {
      result = result.replace(/VALUES\s*\(([^)]+)\)\s*$/gim, 'VALUES ($1) ON CONFLICT DO NOTHING');
    }
    
    // Convert CREATE INDEX IF NOT EXISTS - PostgreSQL supports this
    
    // Convert PRAGMA statements (remove them for PostgreSQL)
    result = result.replace(/PRAGMA\s+[^;]+;?/gi, '');
...
async function insertDefaultData(): Promise<void> {
  ...
  const existingSettings = await db.get('SELECT COUNT(*) as count FROM system_settings');
  ...
  const settings = [
    ...
    ['shipping_free_threshold', '50', 'number', 'shipping', 'Free shipping threshold'],
  ...
  for (const [key, name, description, enabled, killSwitch, category] of features) {
    await db.run(
      `INSERT INTO feature_toggles (feature_key, feature_name, description, is_enabled, is_kill_switch, category) 
       VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (feature_key) DO NOTHING`,
      [key, name, description, enabled, killSwitch, category]
    );
  }
}
