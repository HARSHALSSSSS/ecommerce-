    ...
    <div className="space-y-6 md:space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button 
          ...
          className="inline-flex items-center justify-center sm:justify-start gap-2 px-4 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors font-medium"
        >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 sm:p-5 md:p-6">
        <div className="flex flex-col gap-4">
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg transition-colors font-medium self-start sm:self-auto ${showFilters ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
          >
            <Filter className="w-4 h-4 flex-shrink-0" /> Filters
          </button>
        </div>
        {showFilters && (
          <div className="mt-4 pt-4 border-t border-gray-200 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
            <div>
              <label className="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">SLA Status</label>
              <select 
                value={slaFilter} 
            ...
            <div className="flex items-end">
              <button 
                onClick={() => { setStatusFilter('all'); setSlaFilter('all'); setDateFrom(''); setDateTo(''); setSearch(''); }} 
                className="w-full px-4 py-2.5 text-gray-700 hover:text-gray-900 border border-gray-300 rounded-lg transition-colors font-medium"
              >
                Clear Filters
              </button>
            </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center h-64"><RefreshCw className="w-8 h-8 text-primary animate-spin" /></div>
        ) : orders.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-gray-500">
            <Package className="w-12 h-12 mb-3 text-gray-300" /><p className="text-sm">No orders found</p>
          </div>
        ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Order</th>
                <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Customer</th>
                <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Status</th>
                <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">SLA</th>
                <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap hidden md:table-cell">Items</th>
                <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Total</th>
                <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap hidden lg:table-cell">Date</th>
                <th className="px-4 sm:px-6 py-3 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Actions</th>
              </tr>
            </thead>
              ...
              {orders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 sm:px-6 py-3 whitespace-nowrap">
                    <div className="font-semibold text-gray-900 text-xs sm:text-sm">{order.order_number}</div>
                    <div className="text-xs text-gray-500">ID: {order.id}</div>
                  </td>
                  <td className="px-4 sm:px-6 py-3 whitespace-nowrap">
                    <div className="text-xs sm:text-sm text-gray-900 font-medium truncate">{order.customer_name}</div>
                    <div className="text-xs text-gray-500 truncate">{order.customer_email}</div>
                  </td>
                  <td className="px-4 sm:px-6 py-3 whitespace-nowrap">{getStatusBadge(order.status)}</td>
                  <td className="px-4 sm:px-6 py-3 whitespace-nowrap text-xs">{getSlaIndicator(order)}</td>
                  <td className="px-4 sm:px-6 py-3 whitespace-nowrap text-xs sm:text-sm text-gray-600 hidden md:table-cell">{order.total_items} items</td>
                  <td className="px-4 sm:px-6 py-3 whitespace-nowrap"><div className="font-semibold text-gray-900 text-xs sm:text-sm">{formatCurrency(order.total_amount)}</div></td>
                  <td className="px-4 sm:px-6 py-3 whitespace-nowrap text-xs sm:text-sm text-gray-600 hidden lg:table-cell">
                  ...
                  <td className="px-4 sm:px-6 py-3 whitespace-nowrap text-right">
                    <button 
                      ...
                      className="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors flex-shrink-0 inline-flex" 
                      title="View Details"
                    >
